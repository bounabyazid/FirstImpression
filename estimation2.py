...
X_TR = np.zeros([6000,300])
X_VL = np.zeros([2000,300])
X_TS = np.zeros([2000,300])
...
for key in test:
    ...
    for token in test[key]:
        if token in voc.keys():
           j = j + 1
           X_TS[i][0:300] = X_TS[i] + voc[token]
    X_TS[i] = X_TS[i] / j        
    i = i + 1
    
where_are_NaNs = np.isnan(X_TR)
X_TR[where_are_NaNs] = 0
where_are_NaNs = np.isnan(X_VL)
X_VL[where_are_NaNs] = 0
where_are_NaNs = np.isnan(X_TS)
X_TS[where_are_NaNs] = 0
